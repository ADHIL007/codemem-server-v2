...
use crate::api::types::{
    ...
    GraphEdgeResponse, GraphNodeResponse, LocalEdgeIngestRequest, LocalEdgeIngestResponse,
    MessageResponse, NeighborsQuery, PagerankEntry, PagerankQuery, PagerankResponse,
    ShortestPathQuery, SubgraphQuery, SubgraphResponse,
};
...
fn infer_node_kind(id: &str) -> NodeKind {
    if id.starts_with("file:") {
        ...
        NodeKind::Endpoint
    } else if id.starts_with("author:") {
        // Authors are external entities; use External kind
        NodeKind::External
    } else if id.contains("#class:") || id.contains(":class:") {
        NodeKind::Class
    } else if id.contains("#fn:") || id.contains(":function:") {
        NodeKind::Function
        ...
        NodeKind::Type
    } else if id.contains(":constant:") {
        NodeKind::Constant
    } else if id.contains(":test:") {
        NodeKind::Test
    } else if id.contains(":endpoint:") {
        NodeKind::Endpoint
    } else if id.starts_with("sym:") {
        // Generic sym: prefix — default to Function
        NodeKind::Function
    } else {
        NodeKind::Function
    }
...
/// Ingest lightweight graph edges produced by local TS analysis.
/// This keeps parsing on the client and limits server work to persistence.
pub async fn ingest_local_edges(
    ...
    if req.edges.len() > 20_000 {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(MessageResponse {
                message: "Too many edges in one request (max 20000)".to_string(),
            }),
        ));
    }
    ...
    for e in &req.edges {
        node_ids.insert(e.src.clone());
        node_ids.insert(e.dst.clone());
    }
    ...
    let nodes: Vec<GraphNode> = node_ids
        ...
        .map(|id| {
            ...
            payload.insert(
                "source".to_string(),
                serde_json::Value::String("local-ts".to_string()),
            );
            GraphNode {
                id: id.clone(),
                kind: infer_node_kind(id),
                label: id.clone(),
                payload,
                centrality: 0.0,
                memory_id: None,
    ...
    let mut edges: Vec<Edge> = Vec::with_capacity(req.edges.len());
    for item in &req.edges {
        let relationship = match item.relationship.parse::<RelationshipType>() {
            Ok(r) => r,
            Err(_) => continue,
        };
        ...
        if let Some(file) = &item.file {
            properties.insert("file".to_string(), serde_json::Value::String(file.clone()));
        }
        ...
        edges.push(Edge {
            ...
            dst: item.dst.clone(),
            relationship,
            weight: item.weight.unwrap_or(1.0),
            properties,
            created_at: now,
            valid_from: None,
            valid_to: None,
    ...
    if let Err(e) = storage.insert_graph_nodes_batch(&nodes) {
        return Err((
            ...
            Json(MessageResponse {
                message: format!("Failed to persist nodes: {e}"),
            }),
        ));
    }
    if let Err(e) = storage.insert_graph_edges_batch(&edges) {
        return Err((
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(MessageResponse {
                message: format!("Failed to persist edges: {e}"),
            }),
        ));
    }
    ...
    let mut graph = match state.server.engine.lock_graph() {
        ...
        Err(e) => {
            return Err((
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(MessageResponse {
                    message: format!("Failed to lock graph: {e}"),
                }),
            ))
        }
    };
